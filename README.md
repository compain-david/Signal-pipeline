...
Committed evidence in `analysis/`, regenerated by
[`.github/workflows/analysis.yml`](.github/workflows/analysis.yml). All of it
is **SHADOW ONLY** and governs nothing:

- `analysis/correlation.txt` — signal independence
- `analysis/montecarlo.txt` — whipsaw rates behind the ladder design
- **`analysis/event_study_2021.txt`** — the 2021 altseasons scored against the
  current signal set ([`scripts/event_study.py`](scripts/event_study.py))

The event study's finding is worth reading before citing 2021 as evidence for
any threshold: **only 2 of the 9 Tier A signals (22.2%) can be reconstructed
for 2021 at all**, against a gate needing 5 of 9, and the ladder's 70%
coverage floor holds on **zero of the 365 days of 2021** — it would have been
frozen through both altseasons. CI regenerates it on any change to
`scripts/event_study.py`, but NOT on a retune of `dimensions.py` or
`ladder.py` — what catches that drift is `tests/test_event_study.py`,
which also goes red the day a backfill makes any of those claims false.
